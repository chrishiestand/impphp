<?php
	...
	abstract class ImpDB implements ImpDBO {
		...
		function printQueryLog() {
			...
			<a href="#" class="<?=$QueryTable->Attributes['class']?>" style="position: fixed; bottom: 0; right: 0; text-align: center; width: 10em; padding: 1em 0; background-color: lightgrey; color: black; font-size: 0.8em;" onclick="this.style.display='none';document.getElementById('<?=$className?>_Queries').style.display='block';while(f = <?=$QueryTable->JSRenderQueue?>.shift()){ f(); }; return false"><?=$QueryTable->Caption?> (Show)</a>
			<style>
				#<?=$className?>_Queries {
					text-align: left;
					padding: 2pt;
